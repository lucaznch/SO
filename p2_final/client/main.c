...
int main(int argc, char* argv[]) {
  ...
  setup_data setup_info = ems_setup(argv[1], argv[2], argv[3]);
  ...
  while (1) {
    ...
    switch (next) {
      case CMD_CREATE:
        ...
        if (ems_create(event_id, num_rows, num_columns, setup_info.req_fd, setup_info.resp_fd)) fprintf(stderr, "Failed to create event\n");
        break;
      ...
      case CMD_RESERVE:
        ...
        if (ems_reserve(event_id, num_coords, xs, ys, setup_info.req_fd, setup_info.resp_fd)) fprintf(stderr, "Failed to reserve seats\n");
        break;
      ...
      case CMD_SHOW:
        ...
        if (ems_show(out_fd, event_id, setup_info.req_fd, setup_info.resp_fd)) fprintf(stderr, "Failed to show event\n");
        break;

      case CMD_LIST_EVENTS:
        if (ems_list_events(out_fd, setup_info.req_fd, setup_info.resp_fd)) fprintf(stderr, "Failed to list events\n");
        break;
      ...
      case EOC:
        ems_quit(setup_info.req_fd, setup_info.resp_fd, argv[1], argv[2]);
        close(in_fd);
        close(out_fd);
        return 0;
    }
